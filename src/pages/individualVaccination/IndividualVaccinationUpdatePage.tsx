...
import { individualsVaccinationStore } from './individualVaccinationStore';
...
const Container = styled.div`
  ...
  width: 100%;
`
const IndividualsVaccinationUpdatePage: React.FC = () => {
  const [formData, setFormData] = useState<IndividualsVaccinationInput>({
    individual: individualsVaccinationStore.getIndividual(),
    vaccination: individualsVaccinationStore.getVacccination(),
    date: individualsVaccinationStore.getDate(),
    staff: individualsVaccinationStore.getStaff(),
  });
  ...
  const handleFilterChange = async () => {
      ...
      if (formattedOptions.length !== 0) {
        const a = staffFormattedOptions.find(option => option.label === individualsVaccinationStore.getStaff())
        const b = formattedOptions.find(option => option.label === individualsVaccinationStore.getIndividual())
        const c = vaccinationFormattedOptions.find(option => option.label === individualsVaccinationStore.getVacccination())
        
        setStaffOption(a)
        setIndividualOption(b)
        setVaccinationOption(c)
        ...
        // @ts-ignore
        if (c !== undefined) handleSelectChange(c, { name: 'vaccination' })
      }
  ...
  const handleSubmit = async(e: React.FormEvent) => {
    e.preventDefault();
    const code = await individualsVaccinationService.update(formData, individualsVaccinationStore.getSelf())
    ...
    <>
     <div style={{
      ...
      <Form onSubmit={handleSubmit}>
        ...
        <Button type="submit">Обновить вакцинацию животному</Button>
      </Form>
      </Container>
    </>
    )}
...
export default IndividualsVaccinationUpdatePage;
