...
import { diseaseHistoryStore } from './diseaseHistoreStore';
...
const Container = styled.div`
  ...
  width: 100%;
`
const DiseaseHistoryUpdatePage: React.FC = () => {
  const [formData, setFormData] = useState<DiseaseHistoryInput>({
    dateStart: diseaseHistoryStore.getDateStart(),
    dateEnd: diseaseHistoryStore.getDateEnd(),
    individual: '',
    disease: '',
  });
  ...
  const handleFilterChange = async () => {
      ...
      const diseaseFormattedOptions = (diseaseList || []).map(disease => 
        ...
        const a = diseaseFormattedOptions.find(option => option.label === diseaseHistoryStore.getDisease())
        const b = formattedOptions.find(option => option.label === diseaseHistoryStore.getIndividual())
       
        setDiseaseOptions(a)
        setIndividualOption(b)
        // @ts-ignore
        if (a !== undefined) handleSelectChange(a, { name: 'disease' })
  ...
  const handleSubmit = async(e: React.FormEvent) => {
    e.preventDefault();
    const code = await diseaseHistoryService.update(formData, diseaseHistoryStore.getSelf())
    ...
    <>
     <div style={{
      ...
      <Form onSubmit={handleSubmit}>
        ...
        <Button type="submit">Обновить историю болезни</Button>
      </Form>
      </Container>
    </>
    )}
...
export default DiseaseHistoryUpdatePage;
