...
const Container = styled.div`
  ...
  width: 100%;
`
const FamilyRelationshipsCreatePage: React.FC = () => {
  ...
  const handleSubmit = async(e: React.FormEvent) => {
    e.preventDefault()
    const code = await familyRelationshipService.createFamilyRelationships(formData)
     ...
     <div style={{
      ...
      <Form onSubmit={handleSubmit}>
        <FormGroup>
          ...
          handleSelectChange(option, { name: 'individualId1' })} name="individualId1" 
          options={individual1Options} menuPortalTarget={document.body}
                              styles={{
        ...
        <FormGroup>
          ...
          handleSelectChange(option, { name: 'individualId2' })} name="individualId2" 
          options={individual2Options} menuPortalTarget={document.body}
                              styles={{
                                  menuPortal: base => ({...base, zIndex: 99,}),
                                  control: base => ({...base, width: '98%'})
                              }}/>
        ...
        <FormGroup>
          ...
          handleSelectChange(option, { name: 'typeRelationship' })} name="typeRelationship" 
          options={typeRelationshipOptions} menuPortalTarget={document.body}
                              styles={{
                                  menuPortal: base => ({...base, zIndex: 99,}),
                                  control: base => ({...base, width: '98%'})
                              }}/>
        </FormGroup>
        <Button type="submit">Создать семейные отношения</Button>
      </Form>
      </Container>
    </>
    )}
...
export default FamilyRelationshipsCreatePage;
