...
import { ResponsibleAnimalDto, ResponseAnimal, EmbeddedDto, ResponseAnimalId, ResponseAnimalRequest, ResponseAnimalInput } from "./ResponsibleAnimal.types";
...
export class ResponseAnimalService extends AbstractService {
    ...
    private async getResponseAnimalArray(embeddedDto: EmbeddedDto): Promise<ResponseAnimal[]> {
        ...
        const ResponseAnimalArray: Promise<ResponseAnimal>[] = ResponseAnimalDtos.map(async (ResponseAnimalDto: ResponsibleAnimalDto) => {
            ...
            const self = ResponseAnimalDto._links.self.href;
    ...
    public deleteResponseAnimal(url: string) {
        this.client.delete(`${url}`);
    }

    public createResponseAnimal = async(ResponseAnimal: ResponseAnimalInput) => {
        try {
    ...
    public updateResponseAnimal = async(ResponseAnimal: ResponseAnimal) => {
        try {
        const id: ResponseAnimalId = new ResponseAnimalId(
            ResponseAnimal.dateStart,
            ResponseAnimal.individual,
        ...
        let response = await this.client.put(`${this.baseUrl}/response-animal`, request);
        if (response.data.dateEnd !== undefined) {
            return  'ok'
        } else {
            // @ts-ignore
